...
Athira Technology Web is the public product and services website for a planned, human-reviewed AI Software Engineer made up of seven specialized SDLC agents. It also provides a secure business-enquiry workflow that validates submissions, applies abuse controls, and sends accepted enquiries by email with durable Supabase persistence before email notification.

This repository includes protected staff authentication, RBAC, a Supabase-backed CMS, user management, audit history, and enquiry operations. It does **not** contain production AI-agent execution, customer accounts, analytics, CRM, billing, or subscriptions. Product capabilities remain planned or illustrative unless a page explicitly says otherwise.
...
| `SUPABASE_SECRET_KEY` | Required for contact persistence and user administration | Required | Required | Server-only Supabase secret; never expose to browser code |
| `RESEND_API_KEY` | Required to exercise real local delivery | Recommended | Recommended | Server-only Resend credential; missing notification config does not discard persisted enquiries |
...
Browser ContactForm
  ...
  -> durable Supabase enquiry record
  -> escaped plain-text and HTML email notification
  -> Resend, with the visitor email used only as replyTo
  -> safe 202 response with an ATH reference code
```
...
The route accepts JSON only, limits the UTF-8 body to 16 KiB, does not cache responses, validates the browser origin, and never exposes provider diagnostics, secrets, infrastructure details, or the private recipient. Honeypot submissions receive a neutral accepted response but are neither rate-limited nor emailed. A persistence failure returns a safe 503 and prevents email. After persistence, provider rejection is recorded as a notification failure while the visitor receives the same safe accepted response.
...
Resend is behind a small `ContactEmailProvider` interface and is mocked in automated tests. All user-controlled HTML is escaped. The sender and recipient come only from server configuration, and the validated visitor address is `replyTo`, never `from`. No attachment, auto-reply, customer-visible note, or marketing subscription occurs.
...
| `POST /api/contact` | Validated, rate-limited email delivery; durable Supabase persistence |
...
Contact logs contain only timestamp, request ID, outcome, validation state, rate-limit state, persistence state, provider state, and duration. See the operations runbook for safe Vercel log inspection, abuse/configuration/provider signals, request-ID correlation, and response steps.
