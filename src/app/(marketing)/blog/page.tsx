...
import { listPublishedPosts } from "../../../server/cms/public";
...
export default async function BlogPage() {
  const blogArticles = await listPublishedPosts();
  return (
    <>
      ...
      <Section aria-labelledby="articles-heading">
        <Container>
          <SectionHeading id="articles-heading" eyebrow="Latest articles" title="Build the operating model, not just the demo" description="Published articles are loaded from the content system and ordered by publication date." />
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {blogArticles.map((article) => (
              <article key={article.slug} className="flex h-full flex-col rounded-3xl border border-slate-800 bg-slate-950/50 p-6">
                ...
                <p className="mt-4 flex-1 leading-7 text-slate-400">{article.excerpt}</p>
